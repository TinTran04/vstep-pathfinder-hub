...
## Vocabulary API – Sổ tay từ vựng

> **Trạng thái**: Backend-ready. FE đang dùng `localStorage` qua `vocabulary.mock-service.ts`.  
> Khi có Backend, bật `VITE_DATA_SOURCE=api` để chuyển sang `vocabulary.api-service.ts`.
...
Lưu một từ mới vào sổ tay. Nếu từ đã tồn tại (theo `normalizedWord`), Backend cập nhật `source` và `updatedAt`.
...
* **Request Body**:
  ```json
  {
    "word": "environment",
    "source": "reading",
    "sourceText": "The environment has changed significantly.",
    "sourceUrl": "/attempts/123/review"
  }
  ```
* **Response** `200 | 201`:
  ...
  ```
* **Lưu ý Backend**:
  * Tra cứu nghĩa từ dictionary service/DB và điền `meaningVi`, `phonetic`, `partOfSpeech`, `example`, `audioUrl`.
  * Trả về `200` nếu từ đã tồn tại và được cập nhật; `201` nếu tạo mới.

---

### `DELETE /api/v1/vocabulary/:id`

Xóa một từ khỏi sổ tay.
